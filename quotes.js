...
async function randomQuote() {
  try {
    const res = await fetch("http://127.0.0.1:3000/"); // یا آی‌پی سرور واقعی
    const text = await res.text(); // چون بک‌اند شما متن برمی‌گرداند
    // متن فرمت شده: "Quote" -Author
    const [quotePart, authorPart] = text.split('"-'); // جدا کردن
    const quoteValue = document.getElementById("quote");
    const authorValue = document.getElementById("author");

    quoteValue.textContent = quotePart.replace(/^"/, ''); // حذف " ابتدای متن
